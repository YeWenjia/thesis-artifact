...
Scheme tlist_ind' := Induction for tlist Sort Prop.

Definition tlist_mutind :=
  fun H1 H2 H3 =>
  tlist_ind' H1 H2 H3.

Scheme tlist_rec' := Induction for tlist Sort Set.

Definition tlist_mutrec :=
  fun H1 H2 H3 =>
  tlist_rec' H1 H2 H3.
...
Fixpoint close_exp_wrt_exp_rec (n1 : nat) (x1 : var) (e1 : exp) {struct e1} : exp :=
  match e1 with
    ...
    (* | e_value e2 A1 => e_value (close_exp_wrt_exp_rec n1 x1 e2) A1 *)
    | e_rcd l e => e_rcd l (close_exp_wrt_exp_rec n1 x1 e)
    | e_proj e l => e_proj (close_exp_wrt_exp_rec n1 x1 e) l
    | e_fixpoint e2 => e_fixpoint (close_exp_wrt_exp_rec (S n1) x1 e2)
  end.
...
Fixpoint size_typ (A1 : typ) {struct A1} : nat :=
  match A1 with
    ...
    | t_rcd l A => 1 + (size_typ A) 
    | t_dyn => 1
  end.

Fixpoint size_tlist (AA1 : tlist) {struct AA1} : nat :=
  match AA1 with
    | lt_empty => 1
    | lt_consl AA2 A1 => 1 + (size_tlist AA2) + (size_typ A1)
  end.
...
Fixpoint size_exp (e1 : exp) {struct e1} : nat :=
  match e1 with
    ...
    (* | e_value e2 A1 => 1 + (size_exp e2) + (size_typ A1) *)
    | e_rcd l e => 1 + (size_exp e)
    | e_proj e l => 1 + (size_exp e)
    | e_fixpoint e2 => 1 + (size_exp e2) 
  end.
...
Inductive degree_exp_wrt_exp : nat -> exp -> Prop :=
  ...
  (* | degree_wrt_exp_e_value : forall n1 e1 A1,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (e_value e1 A1) *)
  | degree_wrt_exp_e_rcd : forall n1 e1 l,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (e_rcd l e1)
  | degree_wrt_exp_e_proj : forall n1 e1 l,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (e_proj e1 l)
   | degree_wrt_exp_e_fix : forall n1 e1,
    degree_exp_wrt_exp (S n1) e1 ->
    degree_exp_wrt_exp n1 (e_fixpoint  e1).
...
Inductive lc_set_exp : exp -> Set :=
  ...
  (* | lc_set_e_value : forall e1 A1,
    lc_set_exp e1 ->
    lc_set_exp (e_value e1 A1) *)
  | lc_set_e_rcd : forall e1 l,
    lc_set_exp e1 ->
    lc_set_exp (e_rcd l e1)
  | lc_set_e_proj : forall e1 l,
    lc_set_exp e1 ->
    lc_set_exp (e_proj e1 l)
  | lc_set_e_fix : forall e1,
    (forall x1 : var, lc_set_exp (open_exp_wrt_exp e1 (e_var_f x1))) ->
    lc_set_exp (e_fixpoint e1).
...
Lemma size_tlist_min_mutual :
(forall AA1, 1 <= size_tlist AA1).
Proof.
apply_mutual_ind tlist_mutind;
default_simp.
Qed.

(* end hide *)

Lemma size_tlist_min :
forall AA1, 1 <= size_tlist AA1.
Proof.
pose proof size_tlist_min_mutual as H; intuition eauto.
Qed.

Hint Resolve size_tlist_min : lngen.
...
forall x1 e1,
  lc_exp (open_exp_wrt_exp e1 (e_var_f x1)) ->
  lc_exp (e_fixpoint e1).
Proof.
intros; exp_lc_exists_tac; eauto with lngen.
Qed.

Hint Extern 1 (lc_exp (e_fixpoint _ )) =>
  let x1 := fresh in
  pick_fresh x1;
  apply (lc_e_fix_exists x1) : core.
