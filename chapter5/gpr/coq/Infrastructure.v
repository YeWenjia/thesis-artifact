...
Fixpoint subst_ee (z : var) (u : exp) (e : exp) {struct e} : exp :=
  ...
  | e_unit         => e_unit
  | e_loc l        => e_loc l
  | e_abs e1       => e_abs (subst_ee z u e1)
  ...
  | e_val e A     => e_val (subst_ee z u e) A
  end. *)
...
Definition subst_tb (Z : var) (P : typ) (b : binding) : binding :=
  match b with
  | bind_tvar => bind_tvar
  | bind_typ T => bind_typ (subst_tt P Z T)
  end.
...
(* Hint Resolve subst_tt_fresh : lngen.
Hint Rewrite subst_tt_fresh using solve [auto] : lngen. *)

tsubst_exp_fresh_eq
Lemma subst_te_fresh : forall Z U T,
 Z `notin` fv_te T ->
 T = subst_te Z U T.
Proof with eauto using subst_tt_fresh.
 induction T; simpl; intro H; f_equal...
Qed.
...
type P ->
subst_tt  P X (open_tt_rec k T2 T1) =
  open_tt_rec k (subst_tt P X  T2) (subst_tt P X  T1).
Proof with auto.
...
subst_tt  P X (open_tt_rec k T2 T1) =
  open_tt_rec k (subst_tt P X  T2) (subst_tt P X  T1).
Proof with auto.
 intros T1 T2 X P k WP. 
 apply subst_typ_open_tt_rec; auto.
Qed.

tsubst_typ_open_tt
Lemma subst_tt_open_tt : forall T1 T2 (X:atom) P,
type P ->
subst_tt P X  (open_tt T1 T2) = open_tt (subst_tt  P X T1) (subst_tt P X  T2).
Proof with auto.
intros.
unfold open_tt.
...
tsubst_typ_open_typ_wrt_typ_var
Lemma subst_tt_open_tt_var : forall (X Y:atom) P T,
Y <> X ->
type P ->
open_tt (subst_tt P X  T) Y = subst_tt  P X (open_tt T Y).
...
X `notin` fv_tt T2 ->
open_tt_rec k U T2 = subst_tt  U X (open_tt_rec k (t_fvar X) T2).
Proof with congruence || auto.
...
Lemma subst_tt_intro : forall X T2 U,
  X `notin` fv_tt T2 ->
  open_tt T2 U = subst_tt U X (open_tt T2 (t_fvar X)).
Proof with auto.
  intros.
  unfold open_tt.
...
(* Hint Resolve subst_tt_intro : lngen.
Hint Rewrite subst_tt_intro using solve [auto] : lngen. *)
...
tsubst_exp_open_exp_wrt_exp_var
...
subst_exp_open_exp_wrt_exp_var
...
subst_exp_open_exp_wrt_typ_var
...
Lemma fv_te_open_ee_lower_aux : forall e1 n e2, (*3178*)
fv_te e1 [<=] fv_te (open_ee_rec n e2 e1).
Proof.
apply_mutual_ind exp_mutind;
default_simp; fsetdec.
Qed.
...
fv_exp_open_exp_wrt_exp_rec_lower
...
fv_exp_open_exp_wrt_typ_rec_lower
...
tfv_typ_open_typ_wrt_typ_lower
...
tfv_exp_open_exp_wrt_exp_lower
Lemma fv_te_open_ee_lower :
forall e1 A1,
fv_te e1 [<=] fv_te (open_ee e1 A1).
Proof.
intros.
unfold open_te; simpl; try fsetdec.
apply* fv_te_open_ee_lower_aux.
Qed.

Hint Resolve fv_tt_open_tt_lower fv_tt_open_tt_lower_aux: lngen.


Lemma fv_te_open_te_lower_aux : forall e1 n e2, (*3178*)
fv_te e1 [<=] fv_te (open_te_rec n e2 e1).
Proof.
apply_mutual_ind exp_mutind;
default_simp; fsetdec.
Qed.

tfv_exp_open_exp_wrt_typ_lower
Lemma fv_te_open_te_lower :
forall e1 A1,
fv_te e1 [<=] fv_te (open_te e1 A1).
Proof.
intros.
unfold open_te; simpl; try fsetdec.
apply* fv_te_open_te_lower_aux.
Qed.
...
Fixpoint close_tt_rec (n1 : nat) (X1 : atom) (A1 : typ) {struct A1} : typ :=
  match A1 with
    ...
    | t_unit => t_unit
    | t_dyn => t_dyn
    | t_all B1 => t_all (close_tt_rec (S n1) X1 B1)
    | t_arrow A2 B1 => t_arrow (close_tt_rec n1 X1 A2) (close_tt_rec n1 X1 B1)
    | t_ref A2 => t_ref (close_tt_rec n1 X1 A2)
  end.

Definition close_tt X1 A1 := close_tt_rec 0 X1 A1.



  Lemma subst_tt_mutual :
  (forall A1 A2 A3 X2 X1,
    X2 `notin` fv_tt A2 ->
    X2 <> X1 ->
    subst_tt X1 A2 (subst_tt X2 A3  A1) = subst_tt  X2 (subst_tt  X1 A2 A3)(subst_tt X1 A2  A1)).
  Proof.
  apply_mutual_ind typ_mutind;
  default_simp.
  Qed.
  ...
  forall A1 A2 A3 X2 X1,
    ...
    subst_tt X1 A2 (subst_tt X2 A3 A1) = subst_tt X2 (subst_tt X1 A2  A3)  (subst_tt X1 A2 A1).
  Proof.
  pose proof subst_tt_mutual as H; intuition eauto.
  Qed.
  
  Hint Resolve subst_tt_rec : lngen. *)

  (* Ltac default_auto ::= auto with set lngen; tauto.
  Ltac default_autorewrite ::= autorewrite with lngen.

Lemma fv_close_tt_rec_mutual :
(forall A1 X1 n1,
  fv_tt (close_tt_rec n1 X1 A1) [=] remove X1 (fv_tt A1)).
Proof.
apply_mutual_ind typ_mutind;
default_simp; fsetdec.
Qed.

Hint Resolve fv_close_tt_rec_mutual : lngen.
Hint Rewrite fv_close_tt_rec_mutual using solve [auto] : lngen.

Lemma fv_close_tt :
forall A1 X1,
  fv_tt (close_tt X1 A1) [=] remove X1 (fv_tt A1).
Proof.
  unfold close_tt; default_simp.
Qed.

Hint Resolve fv_close_tt : lngen.
Hint Rewrite fv_close_tt using solve [auto] : lngen.


Lemma typsubst_typ_fresh_same_mutual :
(forall A2 A1 X1,
  X1 `notin` fv_tt A1 ->
  X1 `notin` fv_tt (subst_tt X1 A1 A2)).
Proof.
apply_mutual_ind typ_mutind;
default_simp.
Qed.
...
(forall A1 X1 n1,
  open_tt_rec n1 (t_fvar X1) (close_tt_rec n1 X1 A1) = A1).
Proof.
apply_mutual_ind typ_mutind;
default_simp.
Qed.
...
Lemma subst_tt_spec_mutual :
  (forall A1 A2 X1 n1,
    subst_tt X1 A2  A1 = open_tt_rec n1 A2 (close_tt_rec n1 X1 A1)).
Proof.
  apply_mutual_ind typ_mutind;
  default_simp.
Qed.
  ...
  subst_tt X1 A2  A1 = open_tt_rec n1 A2 (close_tt_rec n1 X1 A1).
  Proof.
  pose proof subst_tt_spec_mutual as H; intuition eauto.
  Qed.

  
  Hint Resolve  subst_tt_spec : lngen. *)
...
Lemma step_not_ires: forall (v:exp) mu mu',
  ires v -> irred v mu mu'.
Proof.
introv.
unfold irred.
inductions v; introv H;
...
Notation "[ z ~> u ] e" := (subst_ee  u z e) (at level 0).
...
(** The two kinds of facts we need about [body_e] are the following:
  ...
  Hint Extern 1 (binds _ (?F (subst_tt ?U ?X  ?T)) _) =>
